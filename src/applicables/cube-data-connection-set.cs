...
public class SendMultiCubeDataOverWiFi : MonoBehaviour
{
    ...
    private const float RECONNECT_INTERVAL = 3f;   // Try every 3 seconds
    ...
    private void ConnectToPC()
    {
        ...
        try
        {
            ...
            bool success = result.AsyncWaitHandle.WaitOne(5000); // 5s timeout
            ...
            {
                Debug.Log($"❌ Connection attempt failed (timeout). Will retry in {RECONNECT_INTERVAL} seconds...");
                isConnected = false;
            }
        }
    ...
    void Update()
    {
        ...
        if (!isConnected)
        {
            ...
            return; // Don't send data while disconnected
        }
        ...
        if (!string.IsNullOrEmpty(data))
        {
            ...
            {
                Debug.LogWarning($"⚠️ Send failed: {e.Message} → Reconnecting...");
                isConnected = false;
            }
        }
    }
    ...
    private void AppendPosRot(System.Text.StringBuilder sb, string prefix, Transform t,
                              ref Vector3 lastPos, ref Vector3 lastRot)
    {
